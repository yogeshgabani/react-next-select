...
const VERSION_HISTORY = [
  {
    ...
    items: [
      ...
      'New `--rns-control-min-height` and `--rns-menu-max-height` CSS variables; `--rns-radius` default bumped from 6px → 10px.',
      'Menu max-height bumped 240px → 320px so more options fit without partial-row cuts at the bottom.',
      'Auto-elevated wrapper (`z-index: 100` when menu is open) so the dropdown escapes parent stacking contexts created by `backdrop-filter`, `transform`, or `opacity` — no more siblings overlapping the menu.',
